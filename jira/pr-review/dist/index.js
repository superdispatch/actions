...
// jira/utils/JiraAPI.ts
var JIRA_BASE_URL = getEnv("JIRA_BASE_URL");
var JIRA_USER_EMAIL = getEnv("JIRA_USER_EMAIL");
var JIRA_API_TOKEN = getEnv("JIRA_API_TOKEN");
...
async function findIssue(input) {
...
async function main() {
  ...
  const shouldApplyAction = !projects || projects.split(",").some((project) => project === issue.fields.project.key);
  if (!shouldApplyAction) {
    (0, import_core.info)(`Skipping... Issue ${issue.key} does not match the specified project filter`);
    return;
  }
  ...
  const { data: pr_reviews } = await octokit.request("GET /repos/{owner}/{repo}/pulls/{pull_number}/reviews", __spreadProps(__spreadValues({}, import_github.context.repo), { pull_number: PR_NUMBER }));
  let senior_approvals = true;
  if (seniors) {
    const filteredChangesRequested = pr_reviews.filter((review) => review.state === "CHANGES_REQUESTED").reduce((acc, cur) => {
    ...
    pr_reviews.forEach((review) => {
      if (review.state === "APPROVED" && review.user?.login && filteredChangesRequested.has(review.user.login) && review.submitted_at && new Date(review.submitted_at) > new Date(filteredChangesRequested.get(review.user.login) || "")) {
...
__name(main, "main");
async function transitionCard(issue, transitionTarget) {
  const jira = createClient();
  const { transitions } = await jira.listTransitions(issue.id);
  const transition = transitions.find((item) => item.name === transitionTarget);
  if (transition) {
    (0, import_core.info)(`Transitioning "${issue.key}" issue to "${transitionTarget}".`);
    await jira.transitionIssue(issue.id, {
      transition: { id: transition.id }
    });
  } else {
    (0, import_core.error)(`Cannot transition "${issue.key}" to "${transitionTarget}".`);
  }
}
__name(transitionCard, "transitionCard");
