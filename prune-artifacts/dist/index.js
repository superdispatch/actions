...
var require_http_client = __commonJS((exports2) => {
  ...
  var HttpClient = class {
    ...
    requestRawWithCallback(info2, data, onResult) {
      ...
      }, req = info2.httpModule.request(info2.options, (msg) => {
...
// index.ts
var import_core = __toModule(require_core()), import_github = __toModule(require_github()), import_util = __toModule(require("util"));
async function main() {
  let token = import_core.getInput("token", {required: !0}), pattern = import_core.getInput("pattern", {required: !0}), skipRecent = parseInt(import_core.getInput("skip-recent"), 10) || 0, octokit = import_github.getOctokit(token), matcher = new RegExp(pattern);
  import_core.info(import_util.format("Iterating through the %s/%s\u2026", import_github.context.repo.owner, import_github.context.repo.repo));
  for await (let {data: artifacts} of octokit.paginate.iterator("GET /repos/{owner}/{repo}/actions/artifacts", {
    ...import_github.context.repo,
    per_page: 50
  })) {
    if (artifacts.length === 0) {
      import_core.info("There are no artifacts left.");
      return;
    }
    import_core.info(import_util.format("Found %s artifacts\u2026", artifacts.length));
    let skipped = 0;
    for (let {id, name} of artifacts) {
      if (!matcher.exec(name)) {
        import_core.info(import_util.format('Skipping "%s" (not matching pattern "%s")', name, pattern));
        continue;
      }
      if (skipRecent > skipped) {
        skipped += 1, import_core.info(import_util.format('Skipping "%s" (belongs to recent "%s")', name, skipRecent));
        continue;
      }
      import_core.info(import_util.format('Removing "%s" artifact with the id "%s"\u2026', name, id)), await octokit.request("DELETE /repos/{owner}/{repo}/actions/artifacts/{artifact_id}", {...import_github.context.repo, artifact_id: id});
    }
  }
}
