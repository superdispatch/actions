...
async function main() {
  ...
  const pattern = import_core.getInput("pattern", {required: true});
  const skipRecent = parseInt(import_core.getInput("skip-recent"), 10) || 0;
  ...
  for await (const {data: artifacts} of octokit.paginate.iterator("GET /repos/{owner}/{repo}/actions/artifacts", {
    ...
    for (const {id, name} of artifacts) {
      ...
      if (skipRecent > skipped) {
        skipped += 1;
        import_core.info(import_util.format('Skipping "%s" (belongs to recent "%s")', name, skipRecent));
        continue;
      }
      import_core.info(import_util.format('Removing "%s" artifact with the id "%s"\u2026', name, id));
      await octokit.request("DELETE /repos/{owner}/{repo}/actions/artifacts/{artifact_id}", {...import_github.context.repo, artifact_id: id});
    }
  }
}
